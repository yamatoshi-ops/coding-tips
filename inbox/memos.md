...
1. **ゼロ相電流抑制のON/OFFで、dqに目立つ次数が 6次 ↔ 12次に変わる**のは不自然ではありません。
   典型的には、

   * **OFF**：3次・9次の零相起因の歪みが、不平衡や空間高調波と混ざって **6次トルク脈動／6次dq脈動** を強く作る
   * **ON**：その主要因が消え、残るのはインバータPWM・デッドタイム・空間高調波・磁気非線形由来の **12次成分** が相対的に卓越する
     という構図です。

2. **軸トルク低下の主因を「鉄損増加だけ」とみなすのは危険**です。
   今の条件だと、優先順位はむしろ
   **(A) 高調波電流による銅損増加 + 高調波磁束による鉄損増加 + トルク生成に寄与しない循環電流の増加**
   の複合と考えるのが自然です。
   とくに「**電流Arms一定なのに有効電力が大きい**」という観測は、**力率改善ではなく、高調波が作る瞬時電力の増加や損失経路の増加**でも説明できます。
...
モータの dq系やトルクで頻出するのは、だいたい次の系列です。
...
これは三相機の対称性と、インバータ・空間高調波・飽和の組合せで自然に出ます。

---

## A. ゼロ相電流抑制OFFで6次が強くなる理由
...
純零相なら dqには直接出ませんが、実機・シミュレーションでは
...
で、これが**完全な零相ではなくなり**、正相/逆相に崩れます。

このとき重要なのは、**3次・9次が基本波と相互作用して 6k次のトルク脈動やdq脈動を作りやすい**ことです。
...
つまり OFF では、零相起因の歪みが大きいので、まず **6次が主役**になりやすいです。

---

## B. ゼロ相電流抑制ONで12次が目立つ理由

抑制ONで 3次・9次がほぼ消えると、6次の主因の一つが消えます。
すると残るのは、
...
このとき、もともと隠れていた **12次** が相対的に目立ちます。

数式的にも、例えば 6次リップルが何らかの非線形を通ると

[
\cos^2(6\omega t)=\frac12\left(1+\cos(12\omega t)\right)
]
...
* **OFF**：ゼロ相電流が“直接の歪み源”として効いている
* **ON**：その歪み源を消したので、“残差としての高次成分”が見えている

です。

つまり、**12次が増えたというより、6次が減って12次が相対的に目立った**可能性が高いです。
絶対値で見るか、比率で見るかで印象が変わります。
...
**鉄損増加は有力候補だが、単独主因と断定しないほうがよい**
です。

今の条件から見ると、軸トルク低下は次の3本立てで考えるのが自然です。

---

## A. まず銅損増加

ゼロ相電流抑制OFFだと、3次・9次の循環電流が流れます。
これらは平均トルク生成に効きにくいのに、

[
P_{cu} \propto I_{\mathrm{rms}}^2 R
]

で損失には効きます。

「電流値[Arms]は一定」とありますが、ここで重要なのは
**何のArmsを一定にしているか**です。
...
もし制御上の有効な基本波成分は同じでも、零相高調波が追加されていれば、**導体内の実効損失は増え得ます**。
その分、同じ入力電力でも軸出力は減ります。
...
[
P_{fe} \sim k_h f B^n + k_e f^2 B^2
]

で増えるので、**9次のような高次成分は特に効きやすい**です。
1000 rpmで基本周波数が低めでも、9次は周波数倍率が大きいので無視しにくいです。

したがって、**鉄損増加は十分あり得る**、むしろかなり有力です。

---

## C. “電磁トルク平均そのもの”の低下

以前整理したように、dqリップルがあると
...
なので、**電圧飽和由来の大きな平均トルク低下**よりは、
**損失増加による軸トルク低下**のほうが主役っぽい印象です。
...
が全部同方向に効いて、合計で約3%になっている可能性が高いです。

---

# 3）「有効電力が大きいのに、モータパワー減少」の解釈

ここは少し言葉を厳密にしたほうがよいです。
...
なら、その差分は**モータ内部損失の増加**です。

つまり

[
P_{in} = P_{shaft} + P_{cu} + P_{fe} + P_{mech} + \cdots
]

なので、

[
P_{in}\uparrow,\quad P_{shaft}\downarrow
]

なら

[
P_{loss}\uparrow\uparrow
]
...
の両方です。
機械損がON/OFFで大きく変わるとは考えにくいので、主犯は電気的損失です。

---

# 4）質問への直接回答

## 1）ゼロ相電流ありなしで、モータ高調波6次と12次と変化するのはなぜ？

最も自然な説明は、

* **抑制OFF**：3次・9次零相が不平衡や非線形で正相/逆相に崩れ、**6次成分**を強く励起する
* **抑制ON**：その主励起源が消え、残るPWM・空間高調波・飽和由来の **12次成分** が相対的に卓越する

です。

つまり、**零相電流があると6次が強く出やすく、消すと高次残差として12次が見える**、という理解でよいです。
...
の順で疑います。
ただしアモルファス/高周波鉄損感度の高い構成なら、2がかなり強くなる可能性はあります。

---

# 5）いま一番効く切り分け

追加の議論に進むなら、次の3つでかなり決着します。

[
P_{cu}=3R_s I_{\mathrm{rms}}^2
]
...
この3つを分ければ、
「3%のうち何%が銅損、何%が鉄損、何%が平均トルク低下か」が見えてきます。

次は、あなたの条件に合わせて
**6次/12次の発生を“空間高調波 × 時間高調波 × Park変換”で整理した表**としてまとめられます。
---
